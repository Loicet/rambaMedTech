...
export default function CaregiverMessages() {
  ...
  const sendReply = () => {
    ...
    const updated = threads.map(th =>
      th.id === selected.id
        ? { ...th, lastMessage: reply, time: 'Just now', messages: [...th.messages, { from: 'caregiver', text: reply, time: 'Just now' }] }
        : th
    );
    setThreads(updated);
  ...
  if (patients.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
        ...
        <p className="text-sm text-gray-400">No patients linked yet. Link a patient to start messaging.</p>
      </div>
    );
  }
          ...
          {threads.map(th => (
            <button key={th.id} onClick={() => setSelected(th)}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-gray-900 truncate">{th.patient}</div>
                ...
                : selected.messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.from === 'caregiver' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed
                      ${msg.from === 'caregiver' ? 'bg-emerald-700 text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'}`}>
                      {msg.text}
                      <div className={`text-[10px] mt-1 ${msg.from === 'caregiver' ? 'text-white/60' : 'text-gray-400'}`}>{msg.time}</div>
                    </div>
                  </div>
                ))
              }
